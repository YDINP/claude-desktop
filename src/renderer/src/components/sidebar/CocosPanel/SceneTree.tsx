...
export interface VirtualSceneTreeProps {
  root: CCSceneNode
  selected: CCSceneNode | null
  onSelect: (n: CCSceneNode | null) => void
  onReparent?: (dragUuid: string, dropUuid: string) => void
...
export function VirtualSceneTree(props: VirtualSceneTreeProps) {
  const {
    root, selected, onSelect, onReparent, onAddChild, onDelete, onDuplicate,
    onToggleActive, hideInactive, favorites, onToggleFavorite, lockedUuids,
    onToggleLocked, nodeColors, onNodeColorChange, collapsedUuids,
    onToggleCollapse, highlightQuery, nodeBookmarks, onReorder,
    multiSelectedUuids, onCtrlSelect, onSortChildren, onRename,
    onSaveAsPrefab, outOfCanvasUuids, scrollContainerRef,
  } = props

  const flatNodes = useMemo(() => {
    const result: FlatNode[] = []
    const walk = (node: CCSceneNode, depth: number, ancestors: string[]) => {
      const isRoot = depth === 0
      const isSelected = selected?.uuid === node.uuid
      if (hideInactive && !node.active && !isRoot && !isSelected) return
      result.push({ node, depth, ancestors })
      const collapsed = collapsedUuids ? collapsedUuids.has(node.uuid) : depth > 2
    ...
    walk(root, 0, [])
    return result
  }, [root, collapsedUuids, hideInactive, selected?.uuid])
...
/** Single row renderer for virtual tree (non-recursive) */
function VirtualSceneTreeRow({
  flatNode, selected, onSelect, onReparent, onAddChild, onDelete, onDuplicate,
  onToggleActive, favorites, onToggleFavorite, lockedUuids, onToggleLocked,
  nodeColors, onNodeColorChange, collapsedUuids, onToggleCollapse,
  highlightQuery, nodeBookmarks, onReorder, multiSelectedUuids, onCtrlSelect,
  onSortChildren, onRename, onSaveAsPrefab, outOfCanvasUuids,
}: {
  flatNode: FlatNode
  selected: CCSceneNode | null
  onSelect: (n: CCSceneNode | null) => void
  ...
  const { node, depth, ancestors } = flatNode
  const [isDragOver, setIsDragOver] = useState(false)
  ...
  const commitNameEdit = useCallback(() => {
    if (onRename && editNameVal.trim()) onRename(node.uuid, editNameVal.trim())
    setEditingName(false)
  }, [onRename, editNameVal, node.uuid])
  ...
  return (
    <>
      ...
      >
        {hasChildren ? (
          <span
            onClick={e => { e.stopPropagation(); onToggleCollapse?.(node.uuid) }}
            style={{ fontSize: 9, width: 12, textAlign: 'center', flexShrink: 0 }}
          >
            {collapsed ? '▸' : '▾'}
          </span>
