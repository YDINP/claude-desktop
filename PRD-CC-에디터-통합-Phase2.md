...
> 최종 업데이트: 2026-03-13 (Round 499 완료 — YY3-1 F키 선택 노드 중앙 포커스)
> 기준: Round 499 완료 상태 (Round 414→499 총 86 라운드 구현 완료)
...
| 472-475 | PRD 업데이트, 컴포넌트 추가 드롭다운(14종), 씬뷰 `<title>` 툴팁, inactive 카운터 |
| 476-479 | Inspector 복제 버튼, 씬뷰 HUD 정렬버튼, 트리 depth 보더, Escape 선택해제 |
| 480-483 | 줌% 클릭 리셋, 우클릭 active 토글, Inspector 씬 파일 정보, Z-order ↑↓ 버튼 |
| 484-487 | 캔버스 밖 노드 반투명(×0.4), 캔버스 외부 빗금 패턴, 치수 레이블, 컴포넌트 fold/unfold |
